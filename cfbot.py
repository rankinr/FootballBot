...
def gameInfo(gm,color=False,showMr=False,score=True,branked=False,custformat='',supershort=False):
	...
	if color:
		...
		if ourgame['team1'].lower() in cis: t1c=str(cis[ourgame['team1'].lower()][0])+','+str(cis[ourgame['team1'].lower()][1])
		if ourgame['team2'].lower() in cis: t2c=str(cis[ourgame['team2'].lower()][0])+','+str(cis[ourgame['team2'].lower()][1])
	t1s=ourgame['team1']
	t2s=ourgame['team2']
...
#	print ourgame['team1'].lower()
	...
	else: return bt+t1.strip()+nident+t2.strip()+'%ABOB%'+mr.strip()+' '+stat_to_show+' '+ntwks.strip()+bt
