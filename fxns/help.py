...
if len(params) != 0:
    ...
    if cached['fxnsdir'].count(params[0] + '.py\n') != 0:
        ...
        if hf[0:3] == '"""':
            hf = hf[hf.find('""') + 3:]
            hf = hf[:hf.find('""')].split('\n')
            db['msgqueue'].append(['Command !' + params[0], origin, None, None,
                                   False])
            for h in hf:
                db['msgqueue'].append([h, origin])
        else:
            db['msgqueue'].append(
                ['I don\'t have any information on that command', origin])
    else:
        db['msgqueue'].append(['I don\'t have any information on that command',
                               origin])
else:
