		...
		if st.count('pm et') != 0:
			...
			if len(nextgames) < 10: nextgames[b]=stp
				...
				if stp < nextgames[mva]:
					nextgames.pop(mva)
					nextgames[b]=stp
		if st.count('final') == 0 and st.count('delay') == 0 and st.count('am et') == 0 and st.count('pm et') == 0 and st.count('cancelled') == 0:
...
ngl=ngl[:-2]
if len(nextgames) != 0: ts+='PRIVMSG '+origin+' :\x02Upcoming games: \x02'+ngl+'\r\n'
comb=', '.join(lstar)
