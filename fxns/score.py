...
if ''.join(params).strip() != '':
	...
	if closests != '' and closestval <= 3:
		...
		db['msgqueue'].append([st_t+gameInfo(closests,True,True,typ_of_text=type_of_text)+'* '+shorten_url('http://sports.espn.go.com/ncf/boxscore?gameId='+teams[closests]['gid'])+'*'+oinfo,msg_dest,msg_type,'score'])
	else:
		conf=[]
		closest_conf=''
			...
			if ''.join(params).lower().replace(' ','') == 'top25':
				...
				for a,b in db['colors'].iteritems():
					...
					if clv_isteam < clv_isteam_val:
						...
						clv_isteam_act=a
				if clv_isteam_val <=3:
					db['msgqueue'].append([origin+': '+clv_isteam_act+' has a bye week.',msg_dest,msg_type,None])
				else:
					...
					if len(pnum) ==4 or len(pnum)==5:
						...
						for a,b in teams.iteritems():
							...
							#return bt+t1.strip()+nident+t2.strip()+mr.strip()+' '+status+bt
							#gm,color=False,showMr=False,score=True,branked=False,custformat='
								if b['status'].count(pnum) != 0 and b['status'].count('EDT') != 0:
