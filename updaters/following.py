...
while time.time() < st+60*4.65 and keepGoing:
	...
	for a,b in following.iteritems():
		if a in db['games'] and len(b) != 0 and db['games'][a]['status'].lower().count('final') == 0 and db['games'][a]['status'].lower().count('am est') == 0 and db['games'][a]['status'].lower().count('pm est') == 0 and db['games'][a]['status'].lower().count('halftime') == 0:
			loopc+=1
			print 'Loading '+a
			...
			#print tg[0]
			if len(tg[0].strip()) != 0 and db['followingrecent'][a] != t1+' '+ourgame['team1score']+nident+t2+' '+ourgame['team2score']+': '+tg[0]:
				msgqueue.append([t1+' '+ourgame['team1score']+nident+t2+' '+ourgame['team2score']+': '+tg[0],a,ourgame['status']])
