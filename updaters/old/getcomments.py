	...
	if score > 5:
		...
		for cmt in cmts:
			...
			if isMain:
				...
				if cscore > 5: scoreval=cscore/((time.time()-created)/60)
				else: scoreval=0
				oldcmts=open('/home/rob/cfb/.oldcmts').read()
				if scoreval > 1: print thiscmt+' ('+str(scoreval)+')'
				if scoreval > 6 and oldcmts.count(':'+cmt['data']['id']+':') == 0:
					#[msg,channel (all main chans), type(privmsg),identifier(None)]
					mqa(['"'+thiscmt+'" -'+cmt['data']['author']+' ('+title+')',None,None,'comment'])
					open('/home/rob/cfb/.oldcmts','a').write(':'+cmt['data']['id']+':')
