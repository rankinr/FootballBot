...
while keepgoing and time.time() < st + 60 * 9.5:
    ...
    for id in ourlisting['data']['children']:
        ...
        #print title
        title = re.sub(r'\([^)]*\)', '', gtr.sub('', title.replace(
            '[', '').replace(']', ''))).strip()
        url = id['data']['url']
        if score > 3:
            ...
            for cmt in cmts:
                ...
                if isMain:
                    ...
                    if scoreval >= 4.2 and oldcmts.count(':' + cmt['data']['id']
                                                             + ':') == 0:
                        #[msg,channel (all main chans), type(privmsg),identifier(None)]
                        #print thiscmt
                        thiscmt = thiscmt.replace('&amt;', '&')
                        cmtsar.append(
                            ['"' + thiscmt + '" -' + cmt['data']['author'] +
                             ' (' + title + ')', None, None, 'comments'])
                        open('/home/fbbot/cfb/updaters/.oldcmts',
                             'a').write(':' + cmt['data']['id'] + ':')
    fdb = open('/home/fbbot/cfb/db.json', 'r')
    fcntl.flock(fdb, fcntl.LOCK_EX)
    db = json.load(fdb)
    if json.dumps(db['games']).count('team1score') == json.dumps(db[
            'games'
    ]).count('PM ET') + json.dumps(db['games']).count('AM ET') or json.dumps(
            db['games']).count('team1score') == json.dumps(db['games']).count(
                    'DELAYED') + json.dumps(db['games']).count(
                            'PM ET') + json.dumps(db['games']).count('FINAL'):
        keepgoing = False
    try:
        for a in cmtsar:
